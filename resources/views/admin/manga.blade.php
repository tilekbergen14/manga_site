                            ...
                            <table class="table-auto w-full">
                                <thead>
                                    <tr>
                                        ...
                                        <th class="border border-black px-4 py-2">Автор</th>
                                    ...
                                    @foreach ($mangas as $manga)
                                        <tr class="hover:bg-teal-100 cursor-pointer">
                                            <td class="border border-black px-4 py-2 font-medium"><img
                                                    class="h-12 aspect-video object-cover" src="{{ $manga->image }}"
                                                    alt="image"></td>
                                            <td class="border border-black px-4 py-2 font-medium">{{ $manga->name }}</td>
                                            <td class="border border-black px-4 py-2 font-medium">
                                                {{ $manga->author ? $manga->author : 'Белгісіз' }}
                                            </td>
                                            <td class="border border-black px-4 py-2 font-medium text-center">
                                                <div class="flex justify-center">
                                                    <form action="{{ route('mangaedit', $manga) }}">
                                                        <button
                                                    ...
                                                    <form action="{{ route('mangadelete', $manga) }}" method="POST">
                                                        @method('delete')
                                                        @csrf
                                                        <button
